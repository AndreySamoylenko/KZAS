...
lowred = np.array([0, 158, 74])
upred = np.array([8, 238, 170])

lowred1 = np.array([172, 140, 60])
upred1 = np.array([255, 230, 180])

lowgreen = np.array([73, 96, 60])
upgreen = np.array([82, 256, 96])
...
def pd_regulator(d1, d2, kp=0.2, kd=1):  # пропорционально-дифференциальный регулятор
...
def cube_r():  # функция поиска красных кубиков
    ...
    if len(contours) != 0:
        ...
        for contor in contours:
            ...
            if area > 250:
                cube_red_exist = time.time()
                if y + h > 95 and flag_wr and cross < 5:  # запись кубиков в список
                    cub_col_list[cross % 4][cub_pos] = 'r'
                    flag_wr = False
                    time_cub_list[cross % 4][cub_pos] = round(time.time() - cross_time, 1)
                    cub_pos = 2
                if y + h < 80 and not flag_wr:
                    flag_wr = True
...
def cube_g():  # функция поиска зеленых кубиков (за подробностями в cube_R())
    ...
    if len(contoursk) != 0:
        ...
        for contork1 in contoursk:
            ...
            if a1 > 300:
                cube_green_exist = time.time()
                if y + h > 95 and flag_wg and cross < 5:
                    cub_col_list[cross % 4][cub_pos] = 'g'
                    flag_wg = False
                    time_cub_list[cross % 4][cub_pos] = round(time.time() - cross_time, 1)
                    cub_pos = 2
                if y + h < 80 and not flag_wg:
                    flag_wg = True
...
while 1:
    ...
    if state == 1:  # езда
        ...
        if green_pos_x == 0 and red_pos_x == 0:  # если нет кубиков
            ...
            if cube_exist_tim + 0.5 > time.time():  # после кубика поворот в направлении движения
                if cube_color == "Green":
                    if color_line == 'orange':
                        degree = -30
                    elif color_line == 'blue':
            ...
            pd_regulator(green_pos_x, (140 + green_pos_y * 1.6), 0.2, 1)

        else:  # если есть красный
            cube_color = "Red"
            cube_exist_tim = time.time()
            # e = (275 - red_pos_y * 1.3) - red_pos_x      # ошибка высчитывается исходя из перспективы
            pd_regulator(red_pos_x, (320 - red_pos_y * 1.6), 0.2, 1)
    ...
    robot.text_to_frame(frame, 'red_pos_x:' + str(red_pos_x), 3, 380)
    robot.text_to_frame(frame, 'red_pos_y:' + str(red_pos_y), 3, 400)
    robot.text_to_frame(frame, 'cubcollist' + str(cub_col_list), 3, 420)
