...
lowblack = np.array([55, 180, 0])
upblack = np.array([86, 256, 75])

lowblue = np.array([90, 80, 34])
upblue = np.array([115, 256, 150])
...
lowred1 = np.array([0, 128, 47])
upred1 = np.array([11, 210, 256])

lowgreen = np.array([70, 225, 58])
upgreen = np.array([83, 256, 250])
...
def cube_g():  # функция поиска зеленых кубиков (за подробностями в cube_R())
    ...
    if len(contoursk) != 0:
        ...
        for contork1 in contoursk:
            ...
            if area > 300:
                cube_green_exist = time.time()
                if y + h > 130 and flag_wg and 0 < cross < 5:
                    cub_col_list[cross % 4][cub_pos % 3] = 'g'
                    flag_wg = False
                    time_cub_list[cross % 4][cub_pos % 3] = round(time.time() - cross_time, 1)
                    cub_pos += 1
                if y + h < 110 and not flag_wg:
                    flag_wg = True
...
while 1:
    ...
    if state == 1:  # езда
        ...
        if green_pos_x == 0 and red_pos_x == 0:  # если нет кубиков
            ...
            if time.time() - cube_red_exist < 0.4:
                kpg = 0.35
            else:
                kpg = 0.25
            cube_color = "Green"
            ...
            pd_regulator(green_pos_x, (150 + green_pos_y * 1.67), kpg, 1)

        else:  # если есть красный
            if time.time() - cube_green_exist < 0.4:
                kpr = 0.3
            else:
                kpr = 0.2
            cube_color = "Red"
            ...
            pd_regulator(red_pos_x, (310 - red_pos_y * 1.67), kpr, 1)
