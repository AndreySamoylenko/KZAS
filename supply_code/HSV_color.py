...
while 1:
    ...
    gblur = cv2.GaussianBlur(imageHSV, (5, 5), cv2.BORDER_DEFAULT)
    mask = cv2.inRange(imageHSV, trackbar_vals['min'], trackbar_vals['max'])
    blur = cv2.blur(mask, (5, 5))
    frame1 = cv2.bitwise_or(frame, frame, mask=blur)
    robot.set_frame(blur, 40)
    # print( trackbar_vals['min'], trackbar_vals['max'])
