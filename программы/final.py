...
def pd_regulator(d1, d2, k_p=5, k_d=4):  # пропорционально-дифференциальный регулятор
        ...
        for contork1 in contoursk:
            ...
            if area > 450:
                ...
                if cube_green_exist + 0.15 < time.time():
                    xm, ym, wm, hm = 0, 0, 0, 0
        if ym > 100 and flag_wg and 0 < cross < 5:
            cub_col_list[cross % 4][cub_pos % 3] = 'g'
            flag_wg = False
            time_cub_list[cross % 4][cub_pos % 3] = round(time.time() - search_cross_time, 1)
            cub_pos += 1
        if ym < 50 and not flag_wg:
            flag_wg = True
    else:
        if cube_green_exist + 0.15 < time.time():
            xm, ym, wm, hm = 0, 0, 0, 0
...
while 1:
    ...
    if state == 2:  # разворот
        if color_line == "blue":
            if reverse_timer + 1.6 > time.time():
                speed = 30
                degree = -60
            elif reverse_timer + 3.0 > time.time():
                speed = -30
                degree = 60
            elif reverse_timer + 3.5 > time.time():
                speed = 40
                degree = 0
            else:
                state = 1
    ...
    if cross == cross_reverse and last_cube_color == "red":
        state = 2
        cross = 9
