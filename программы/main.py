...
servo.angle(-8)  # поворот сервы в 0 градусов
...
def motor(sped):  # функция упрощённого управления мотором
    ...
    if sped > 0:
        ...
        Mb.low()
        Ma.high()
        ch.pulse_width_percent(abs(sped))
            ...
            if not flag_start:  # сообщение "99999999999999999999" говорит о том что распберри запустилась
                ...
                try:
                    if inn != '99999999999999999999' and len(inn) == 20:  # чтение сообщения и езда
                        ...
                        servo.angle(rul)  # поворачиваем сервопривод на заданный угол
                        # rul - 8 нужно для того, чтобы угол сервы совпадал с 0 переменной rul
                    else:
                        RGB_LED(99, 99, 99)
                        print(inn)
                except ValueError:
